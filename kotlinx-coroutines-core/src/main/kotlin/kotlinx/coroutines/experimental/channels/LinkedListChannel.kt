...
public open class LinkedListChannel<E> : AbstractChannel<E>() {
    protected final override val hasBuffer: Boolean get() = true
    protected final override val isBufferEmpty: Boolean get() = isEmpty
    ...
    protected override fun offerSelectInternal(element: E, select: SelectInstance<*>): Any {
        while (true) {
            val closed = closedForReceive
            if (closed != null) return closed
            val result = if (hasReceiveOrClosed)
                super.offerSelectInternal(element, select) else
                select.performAtomicTrySelect(describeSendBuffered(element))
            when {
